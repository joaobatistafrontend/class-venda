...
class EditarVendaView(View):
    ...
    def get(self, request, pk):
        ...
        todos_produtos = Produto.objects.all()  # Obter todos os produtos disponíveis
    ...
    def post(self, request, pk):
        ...
        if 'adicionar' in request.POST:
            ...
            # Lógica para adicionar o produto à venda
            produto = get_object_or_404(Produto, pk=produto_id)
            VendaDoProduto.objects.create(venda=venda, produto=produto, qtd=quantidade, total=quantidade * produto.valor)

        elif 'remover_da_venda_editada' in request.POST:
            produto_id = request.POST.get('produto_id')
            venda_produto = get_object_or_404(VendaDoProduto, venda=venda, produto_id=produto_id)
            venda_produto.delete()  # Remove o produto da venda

        return redirect('editarVendaProduto', pk=pk)  # Redireciona para a mesma página após a alteração