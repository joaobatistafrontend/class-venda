...
class EditarVendaView(View):
    ...
    def get(self, request, pk):
        ...
        # Criando um FormSet para os itens da venda
        VendaDoProdutoFormSet = modelformset_factory(VendaDoProduto, fields=('produto', 'qtd'), extra=1, can_delete=True)
    ...
    def post(self, request, pk):
        venda = get_object_or_404(Venda, pk=pk)
        # Criando o mesmo FormSet que no GET para processar os dados
        VendaDoProdutoFormSet = modelformset_factory(VendaDoProduto, fields=('produto', 'qtd'), extra=1, can_delete=True)
        formset = VendaDoProdutoFormSet(request.POST, queryset=venda.itens.all())

        if formset.is_valid():
            # Salva cada item no formset
            items = formset.save(commit=False)
            for item in items:
                item.venda = venda  # Garantindo que cada item esteja associado à venda atual
                item.save()

            # Deletar itens que foram marcados para exclusão no formset
            ...
            messages.error(request, "Houve um erro ao atualizar a venda. Verifique os dados e tente novamente.")
