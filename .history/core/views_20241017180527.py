...
class NovaVendaView(CreateView):
    ...
    def post(self, request):
        ...
        produto_form = VendaDoProdutoForm(request.POST)

        if venda_form.is_valid() and produto_form.is_valid():
            # Salva a nova venda
            nova_venda = venda_form.save()

            # Salva o produto associado à venda
            produto_venda = produto_form.save(commit=False)
            produto_venda.venda = nova_venda  # Associa o produto à venda recém-criada
            produto_venda.save()

            messages.success(request, 'Venda e produto adicionados com sucesso!')
