...
class EditarVendaView(View):
    ...
    def get(self, request, pk):
        ...
        # Formset para os produtos já presentes na venda
        VendaDoProdutoFormSet = modelformset_factory(
            VendaDoProduto, fields=('produto', 'qtd'), extra=0, can_delete=True
        )
        formset = VendaDoProdutoFormSet(queryset=venda.itens.all())

        # Recupera todos os produtos para exibição no formulário de adição de novos produtos
        produtos = Produto.objects.all()
    ...
    def post(self, request, pk):
        ...
        # Formset para atualizar os produtos já existentes na venda
        VendaDoProdutoFormSet = modelformset_factory(
            VendaDoProduto, fields=('produto', 'qtd'), extra=0, can_delete=True
        )
        formset = VendaDoProdutoFormSet(request.POST, queryset=venda.itens.all())

        if formset.is_valid():
            # Salva as alterações feitas nos produtos já existentes
            items = formset.save(commit=False)
            for item in items:
                item.venda = venda  # Garante que cada item esteja vinculado à venda
                item.save()
        ...
        # Adiciona novos produtos à venda se selecionados no formulário
        produto_id = request.POST.get('produto_id')
        ...
        if produto_id and quantidade:
            ...
            # Verifica se o produto já existe na venda para incrementar a quantidade
            item_venda, created = VendaDoProduto.objects.get_or_create(
                venda=venda, produto=produto
            )
            if created:
